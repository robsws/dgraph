...
type SubGraph struct {
	...
	facetsMatrix map[uint64]map[uint64]*pb.Facets
...
func evalLevelAgg(
	...
	for srcUID, uidSet := range relSG.uidMatrix {
		...
		uidSet.Iterate(func(adjUID uint64) error {
...
func (fromNode *varValue) transformTo(toPath []*SubGraph) (map[uint64]types.Val, error) {
	...
	for ; idx < len(toPath); idx++ {
		...
		for srcUID, adjUIDSet := range curNode.uidMatrix {
			...
			adjUIDSet.Iterate(func(dstUID uint64) error {
...
func (sg *SubGraph) updateFacetMatrix() {
	...
	for srcUID, adjUIDSet := range sg.uidMatrix {
		...
		if !adjUIDSet.IsEmpty() {
			codec.Difference(sg.DestUIDs, adjUIDSet).Iterate(func(destUID uint64) error {
				// If uid wasn't filtered then we keep the facet for it.
				delete(sg.facetsMatrix[srcUID], destUID)
				return nil
			})
		}
	}
...
func (sg *SubGraph) populateFacetVars(doneVars map[string]varValue, sgPath []*SubGraph) error {
	...
	for srcUID, uids := range sg.uidMatrix {
		uids.Iterate(func(uid uint64) error {
			facet := sg.facetsMatrix[srcUID][uid]
...
func (sg *SubGraph) applyIneqFunc() error {
	...
	if sg.SrcUIDs != nil {
		...
		*/
		// FIXME: Why/how can there be a non-empty sg.uidMatrix without srcUIDs?
		// Temporarily using srcUID of 0
	...
	for i := 0; i < len(sg.Children); i++ {
		...
		default:
			if len(child.ExpandPreds) > 0 {
				...
				valueLists := make([]*pb.ValueList, 0, len(child.ExpandPreds))
				for _, valueList := range child.ExpandPreds {
					valueLists = append(valueLists, valueList)
				}
				preds = getPredsFromVals(valueLists)
			} else {
...
func ProcessGraph(ctx context.Context, sg, parent *SubGraph, rch chan error) {
	...
	case parent == nil && sg.SrcFunc != nil && sg.SrcFunc.Name == "uid":
		...
		if sg.SrcUIDs != nil && !sg.SrcUIDs.IsEmpty() {
			...
			// Populated variable.
			// TODO: Check if below is correct. Used 0 again
			sg.uidMatrix = map[uint64]*codec.UIDSet{0: sg.DestUIDs.Clone()}
			/*
				sort.Slice(sg.DestUIDs.Uids, func(i, j int) bool {
					return sg.DestUIDs.Uids[i] < sg.DestUIDs.Uids[j]
