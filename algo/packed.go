...
func IntersectWithLinPacked(uidPack1, uidPack2 *pb.UidPack) *pb.UidPack {
	...
	intersection := codec.NewEncoder()
	if len(uidPack1.Blocks) > 0 && len(uidPack2.Blocks) > 0 {
		dec1 := codec.NewDecoder(uidPack1)
		dec2 := codec.NewDecoder(uidPack2)

		for dec1.Valid() && dec2.Valid() {
			if dec1.CurrentBase() > dec2.CurrentBase() {
				dec2.Next()
			} else if dec1.CurrentBase() < dec2.CurrentBase() {
				dec1.Next()
			} else {
				// TODO: Consider using roaring.ParAnd below
				rb := roaring.And(dec1.UnpackBlockRoaringBitmap(), dec2.UnpackBlockRoaringBitmap())
				intersection.AddBlockFromBitmap(dec1.CurrentBase(), rb, uint32(rb.GetCardinality()))
				dec1.Next()
				dec2.Next()
			}
		}
